...
/*
  ...
  输入 :
    SourceText : 资源数据（必须是一个变量，不是指针，或者句柄）
    SourceSize : 数据大小
    DestinationMemory : 输出变量
    NbBytesPerLine : 每行的字节数，行间通过CR/LF分隔。指定0xFFFFFFFF设置不换行。
    UseAdobeEncoding (可选) : 设置1，则在编码后的数据中使用Adobe前后缀"<~" "~>"，此时在解码时大小要+5。默认0，不使用。
  输出 :
    返回：输出变量数据大小
*/
...
/*
  ...
  输入 :
    SourceText : 编码后的数据
    DestinationMemory : 输出变量-解码后数据
    SourceSize (optional) : 需解码的数据大小，如果省略，则编码数据*必须*是以\0结尾的。
  输出 :
    返回：输出变量数据大小
*/
...
/*
  ...
  输入 :
    ...
    SourceSize : 需解码的数据大小
  输出 :
    如果数据不是纯的Ascii85编码，返回-2
    解码出错，返回-1，比如编码不完整
    解码成功，返回返回解码后数据大小（ >=0）
  注 :
    如果对解码数据大小不确定，则调用这个函数，如果对数据大小有把握（毕竟是你自己编码的），可以跳过这步直接解码
*/
